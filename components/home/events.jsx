...
async function getEvent() {
	const res = await fetch(`${process.env.URL}/api/events?filter=active&limit=1`, { next: { tags: ['events'] }, cache: 'no-store' })
		...
		<div className="py-8 border-b border-b-hlinoteka-special">
			<div className="px-2 py-8 sm:p-12 lg:px-24 lg:py-20 max-w-8xl mx-auto">
				...
				<div className="mt-4 sm:mt-8 w-full text-center">
					<Link href="/udalosti">
						<Button type="secondary" className="w-full sm:w-auto text-xs">Další události</Button>
					</Link>
				</div>
			</div>
		</div>
