...
export default function ContactForm() {
  ...
  return (
      <div className="flex flex-col justify-center text-center gap-6 p-4">
        ...
        {message ? (
          ...
          <form className="flex flex-col justify-center gap-5 w-full mx-auto" onSubmit={sendEmail}>
            ...
            <div className="flex place-items-center gap-4">
              ...
              <button
                className={`bg-hlinoteka-dark border-2 border-hlinoteka-special rounded-full ${checked ? "p-2 bg-hlinoteka-special" : "p-4"}`}
              ...
              <label htmlFor="agree" className={`${dmSans.className}`}>
                Souhlasím se{" "}
                <a className="underline hover:no-underline" href="/privacy" target="_blank">
                  zpracováním osobních údajů
                </a>
              </label>
            </div>
            <div className="w-auto">
              <Button type={`${checked && !loading ? "primary" : "disabled"}`} className="w-full sm:w-auto text-xs" loading={loading}>
                Odeslat
              </Button>
            </div>
          </form>
